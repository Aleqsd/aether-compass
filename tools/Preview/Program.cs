...
internal static unsafe class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        PreviewCase[] cases = [
            new("goals-level100", 790, 740),
            new("weekly-unknown", 790, 740, Weekly: true, KnownCurrency: false),
            new("logged-out", 790, 410, LoggedIn: false),
            new("minimum-scale150", 590, 650, 1.5f, Weekly: true, KnownCurrency: false),
        ];
    ...
    private static void Render(PreviewCase preview, string output)
    {
        ...
        try
        {
            ...
            var window = new MainWindow(config, new ProgressStore(), () => { }) { RequestWeekly = preview.Weekly };
            ...
            for (var frame = 0; frame < 4; frame++)
            {
                ...
                ImGui.SetNextWindowSize(new Vector2(preview.Width, preview.Height) * preview.Scale);
                var open = true;
                // WindowSystem requires running Dalamud services. This is the same Draw() body.
                ImGui.Begin("Aether Compass###AetherCompass", ref open, ImGuiWindowFlags.None);
                window.Draw();
